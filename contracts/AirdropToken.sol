...
contract AirDropToken {
    ...
    constructor(string memory name, string memory symbol, uint8 decimals, bytes32 rootHash, uint256 premine) public {
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
        _rootHash = rootHash;
    ...
    function redeemPackage(uint256 index, address recipient, uint256 amount, bytes32[] memory merkleProof) public {
        ...
        uint256 redeemedBlock = _redeemed[index / 256];
        uint256 redeemedMask = (uint256(1) << uint256(index % 256));
        require((redeemedBlock & redeemedMask) == 0, "Already redeemed");
        ...
        require(node == _rootHash, "Invalid recipient");
